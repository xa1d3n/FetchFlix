...
class TMDBClient: NSObject {
    ...
    func escapeTitle(title: String?) -> String {
        var escapedString = ""
        if let title = title {
            escapedString = title.stringByAddingPercentEncodingWithAllowedCharacters(.URLHostAllowedCharacterSet())!
        }
        
        return escapedString
    }
    
    // GET info FROM NETFLIX ROULETTE
    func taskForGETNetflixRoullete(title : String, completionHandler: (result: AnyObject!, error: NSError?) -> Void) -> NSURLSessionDataTask {
        let escapedTitle = escapeTitle(title)
        let urlString = Constants.NetflixRouletteURL + escapedTitle
        let url = NSURL(string: urlString)
        let request = NSURLRequest(URL: url!)
        
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            guard error == nil else {
                print("there was error with netflix roullete \(error)")
                return
            }
            guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 299 else {
                if let response = response as? NSHTTPURLResponse {
